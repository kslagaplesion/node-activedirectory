...
defaultAttributes = originalDefaultAttributes = {
  user: [
    'dn', 'userPrincipalName', 'sAMAccountName', 'whenCreated', 'pwdLastSet',
    'userAccountControl', 'sn', 'givenName', 'cn', 'displayName', 'adminCount',
    'badPwdCount', 'lastLogon', 'logonCount', 'mail', 'pwdLastSet',
    'telephoneNumber', 'whenChanged', 'facsimileTelephoneNumber', 'accountExpires',
    'pager', 'mobile', 'jobTitle', 'department', 'company', 'street', 'city',
    'extensionAttribute1', 'extensionAttribute2', 'extensionAttribute3', 'extensionAttribute4',
    'extensionAttribute6', 'extensionAttribute9', 'extensionAttribute10', 'extensionAttribute11'
  ],
  group: [
    'dn', 'cn', 'description'
  ]
...
ActiveDirectory.prototype.findGroupsByUser = function findGroupsByUser(opts, callback) {
  var self = this;
  ...
  var localOpts = _.defaults(_.omit(opts || {}, 'attributes'), {
    scope: 'sub',
    attributes: joinAttributes((opts || {}).attributes || [], defaultAttributes.group || [],
  ...
  search.call(self, localOpts, function onFind(err, results) {
    ...
    async.forEach(results[0].memberOf, function(item, asyncCalback) {
      var items = item.split(',');
      var items = items[0].split('=');

      result.groups.push(items[1]);
    }, function(err) {
      console.log(err);
    });
...
ActiveDirectory.prototype.addUser = function addUser(baseDN, username, password, opts, callback) {
  var self = this;
  ...
  client.bind(username, password, function(err) {
    ...
    client.add('cn=' + user.cn + ',' + baseDN, user, (err, response) => {
      if(err) {
        return callback(err);
      }
